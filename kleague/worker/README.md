# 국내 프로토 배당 웹 (kl.usb.kr)

베트맨 프로토 승부식의 국내 경기 배당(축구 야구 농구 배구)을 모아 보여주는 페이지입니다.
위쪽 종목 버튼(`/?sport=야구` 등)으로 종목을 고릅니다.

```
국내 PC (betman_collector.py) ──POST /ingest──▶ Cloudflare Worker ──▶ D1 kleague-odds
                                                     │
                                          https://kl.usb.kr 에서 조회
```

베트맨은 해외 IP를 막습니다. 그래서 수집은 국내 PC가 하고 Worker는 저장과 화면만 맡습니다.

## 1. Worker 배포 (한 번만)
D1 데이터베이스와 테이블은 이미 만들어 두었습니다.
```bash
cd kleague/worker
npm install
npx wrangler login
npx wrangler secret put INGEST_TOKEN   # 수집기와 같이 쓸 긴 임의 문자열
npm run deploy
```
DNS에 `kl` 레코드가 이미 있으면 배포가 실패하니 대시보드에서 먼저 지워야 합니다.

## 2. 국내 PC에서 수집기 실행
Python 3.8 이상이 필요합니다. 베트맨은 브라우저가 아닌 접속을 끊으므로 Chrome 처럼 접속하는 `curl_cffi` 를 설치합니다.
```bash
python -m pip install curl_cffi
cd kleague
python3 betman_collector.py --dry-run          # 전송 없이 동작 확인
export INGEST_URL=https://kl.usb.kr/ingest
export INGEST_TOKEN=위에서_정한_토큰
python3 betman_collector.py
```
- 최근 회차를 자동으로 찾아갑니다. 마지막 회차는 `.betman_state.json` 에 저장됩니다
- 직전 회차도 같이 받아서 경기 결과가 반영됩니다
- 국내 경기가 없는 회차에는 보낼 것이 없어서 대상 0행으로 끝납니다
- 다른 리그로 시험하려면 `--league "아시안게임"` 처럼 리그명 정규식을 주면 됩니다

### 주기 실행 (리눅스 cron 예시)
```cron
*/30 * * * * cd /path/to/SouP/kleague && INGEST_URL=https://kl.usb.kr/ingest INGEST_TOKEN=토큰 python3 betman_collector.py >> betman.log 2>&1
```
로그는 UTF-8 이라 PowerShell 에서는 `Get-Content betman.log -Encoding utf8` 로 봐야 글자가 깨지지 않습니다.

Windows는 `kleague/run_betman.bat` 에 토큰을 넣고 `install_task.bat` 을 한 번 실행하면 30분마다 돌아갑니다. 토큰을 넣은 bat 파일은 커밋하지 마세요.
수집기는 시작 전 0~5분 무작위로 기다리고 한 번에 2~3회차만 조회합니다. 모든 요청이 실패하면 2시간부터 최대 24시간까지 스스로 쉽니다.

## 과거 회차 일괄 수집
프로토는 마감된 회차도 최종 배당과 결과를 돌려줍니다. 2021년 1회차부터 지금까지를 천천히 받아 둡니다.
```bash
python3 betman_collector.py --backfill 2021            # 한 번 실행에 4000행까지 이어받기
python3 betman_collector.py --backfill 2021 --dry-run  # 전송 없이 확인
```
- 한 번 실행에 보내는 행을 4000개(`--max-rows`)까지로 막습니다. 새 행 하나가 D1 쓰기 약 6번이라 하루 3번이면 최대 약 7만 2천입니다
- 진행 위치는 `.betman_backfill.json` 에 저장되어 다음 실행이 이어서 합니다
- 요청 사이 8~20초를 쉬고 빈 회차가 3번 이어지면 다음 해로 넘어갑니다
- 접속이 3번 연속 실패하면 6시간 쉽니다
- Windows는 `install_backfill.bat` 을 한 번 실행하면 8시간마다 이어받습니다. 토큰은 `run_betman.bat` 에서 읽습니다
- 과거 회차는 마감 배당만 남습니다. 배당 변동 이력은 실시간 수집분에만 있습니다

## 분석용 내보내기
수집 대상은 베트맨 프로토의 모든 경기(국내와 해외 전 종목)입니다. 화면에는 국내 리그만 보이고 `/api/upcoming` 은 해외까지 전부 줍니다.

## D1 무료 한도
| 항목 | 한도 (하루) | 예상 |
|---|---|---|
| 쓰기 | 10만 행 | 실시간 약 1만 + 과거 회차 최대 7만 2천 |
| 읽기 | 500만 행 | 화면과 API 최대 약 150만 + 통계 하루 한 번 약 150만 |

- 새 행: 경기 3 + 배당 3 쓰기. 바뀐 것이 없으면 0. 상태만 바뀌면 1 (2026-09 원격 D1 실측)
- `/api/upcoming`: 앞으로 14일과 지난 3일 경기 전부. 경기마다 `league` `sport`(축구 야구 농구 배구 기타) `domestic`(true 국내 / false 해외) 와 게임 유형별 배당 `bets` 가 있습니다. `?scope=domestic` 이면 국내만, `?sport=야구` 면 그 종목만
- 화면과 `/api/upcoming` 은 10분 캐시. 적중률과 추천 통계는 하루 한 번 SQL 로 묶어 계산해 D1 `kv` 표의 `stats` 에 둡니다
```
https://kl.usb.kr/export.csv                                  # 최근 100회차 전체
https://kl.usb.kr/export.csv?gm_from=210001&gm_to=210100      # 회차 범위 (한 번에 100회차)
https://kl.usb.kr/export.csv?league=EPL,라리가                 # 리그 짧은 이름
https://kl.usb.kr/api/rounds                                  # 회차별 리그별 경기 수
```
게임 유형마다 첫 배당(w0 d0 l0)과 마지막 배당(w d l)과 결과와 점수가 한 줄씩 들어 있습니다. 엑셀에서 바로 열립니다.

## 배당 변경 표시
- 맨 위 **최근 24시간 배당 변경** 목록에 바뀐 경기와 이전 값→새 값이 최신순으로 나옵니다
- 바뀐 경기 카드는 주황 테두리와 `배당 변경 N건` 배지가 붙습니다
- 바뀐 칸은 노란 배경에 이전 값이 취소선으로 함께 보입니다. 핸디캡과 언더오버 기준값이 바뀌면 `2.5→3.5` 처럼 보입니다
- 처음 발표된 배당은 `새 배당` 배지가 붙습니다

## 구매할 때
- 게임 이름 앞 숫자는 **베트맨 경기 번호**입니다. 베트맨 화면이나 판매점 용지에서 이 번호로 찾습니다
- 경기 카드의 **베트맨 구매** 링크는 그 회차의 베트맨 구매 화면을 엽니다. 끝난 경기는 **베트맨 결과** 로 마감 회차 화면을 엽니다
- **경기 정보** 링크는 네이버에서 두 팀 경기 정보를 검색합니다
- 맨 위 배당 변경 목록의 경기 이름을 누르면 그 경기 카드로 이동합니다
- `단폴` 배지가 있으면 그 경기 하나만 살 수 있습니다. 없으면 다른 경기와 2경기 이상 묶어야 합니다

## 추천 베팅
- 화면 위 **추천 베팅** 에 지금 배당 중 과거에 비슷한 배당이 실제로 더 자주 맞았던 선택을 기대수익 순으로 보여 줍니다. 경기 카드에서는 보라색 테두리와 `추천 +N%` 로 표시됩니다
- 묶음: 같은 종목 · 같은 게임 유형 · 같은 쪽(승 무 패 언더 오버) · 비슷한 배당 구간. 묶음의 실제 적중률과 배당 확률의 비율로 지금 확률을 보정합니다
- 조건: 기대수익 3% 이상 · 같은 묶음 과거 60번 이상. 표본이 적으면 배당 확률 쪽으로 끌어당겨 우연에 덜 흔들리게 합니다
- **과거 검증**: 시간 순서대로 그 시점까지의 데이터만으로 추천했다면 어땠는지 계산해 함께 보여 줍니다. 이 값이 + 가 되기 전까지는 참고만 하세요
- 추천 근거와 과거 검증은 국내 경기 기록만 씁니다. 과거 검증은 시즌 단위로 앞 시즌까지의 기록으로 다음 시즌을 맞혀 봅니다
- `/api/picks` 로 검증 결과와 과거에 후했던 배당 구간을 JSON 으로 받습니다. 하루 한 번 새로 계산합니다

## 구매 가능 표시
- 경기 카드와 베팅할 곳에 `구매 가능 · 18:50 마감 (2시간 10분 남음)` / `발매 전 · 배당 미발표` / `구매 마감` 이 붙습니다
- 베트맨은 경기 시작 10분 전에 발매를 마감합니다. 배당이 나왔으면 발매 중으로 보고 마감까지 남은 시간은 화면이 30초마다 지금 시각으로 다시 계산합니다
- 마감 1시간 전부터는 주황색으로 바뀝니다
- 베트맨이 경기를 발매 취소하거나 시각을 바꾼 것은 다음 수집 때 반영됩니다

## 경기 중 실시간 상태
- 진행 중인 국내 경기 카드에 `● LIVE 강원FC 1:0 인천 · 후반 12분` 처럼 점수가 나오고 그 경기의 추천이 **지금이면 적중 / 지금이면 실패 / 동점** 인지 보여 줍니다
- 언더오버의 오버처럼 이미 결과가 정해진 경우는 **적중 확정 / 실패 확정** 으로 나옵니다
- 맨 아래 결과 기록의 **대기** 칸도 같은 상태로 바뀝니다
- 점수는 네이버 스포츠 일정 API(K리그 KBO KBL WKBL V리그)에서 받고 `/api/live` 가 1분 캐시합니다. 화면이 1분마다 새로 불러옵니다
- 배구 핸디캡과 언더오버는 세트 점수로 판단할 수 없어서 표시하지 않습니다

## 추천 베팅 결과 기록
- 페이지 맨 아래 **추천 베팅 결과 기록** 에 추천을 경기마다 10만원씩 샀다고 치고 결과를 남깁니다
- 수집기가 `/notify` 를 부를 때(30분마다) 앞으로 3일 안 국내 경기의 추천을 D1 `pick_log` 에 적습니다. 경기 전 마지막 확인까지 추천이었던 것만 넣고 배당은 그때 마지막 배당입니다
- 베팅 수 · 적중률 · 손익 · 수익률 · 결과 대기 칸과 누적 손익 선 그래프와 최근 40건 표를 보여 줍니다

## 예상 vs 결과
- 화면 아래 **배당 예상 vs 실제 결과** 에서 국내 리그 기본 게임(축구 승무패 야구 농구 배구 승패) 마감 배당이 실제로 얼마나 맞았는지 봅니다
- 예상은 마진을 뺀 배당 확률이고 실제는 적중 비율입니다. 차이가 + 면 배당이 예상한 것보다 자주 맞았다는 뜻입니다
- 수익률은 매 경기 같은 금액을 걸었을 때 기준입니다. 손익은 경기마다 10만원을 걸었을 때 총 금액입니다 (만원)
- 정배와 무승부와 역배 그리고 정배 배당 구간별과 시즌별로 나눠 보여 줍니다
- 끝난 경기 카드에는 `정배 적중` 또는 `정배 X 실패` 배지와 정배에 10만원 걸었을 때 손익이 붙습니다 (예: `10만원 → +134,000원`)
- 종목 버튼을 누르면 그 종목만 계산하고 리그별 정배 표도 나옵니다. 하루 한 번 새로 계산합니다
- JSON 은 `/api/accuracy` (`?sport=야구`) 로 받습니다

## K리그 카카오톡 알림
수집기가 30분마다 돌 때 K리그 경기를 보고 카카오톡 **나와의 채팅**으로 보냅니다. 메시지는 모두 `[K리그]` 로 시작합니다.
- **새 배당**: 경기 배당이 처음 올라왔을 때
- **배당 변동**: 마지막으로 알린 값보다 승 무 패 중 하나가 0.1 이상 바뀌었을 때
- **경기 전 요약**: 경기 18시간 안으로 들어오면 마지막 배당과 정배 확률과 핸디캡 언더오버 요약 (수집 PC 가 21~05시에만 켜져 있어서 낮 경기는 전날 밤 배당 기준)
- **결과**: 점수와 정배 적중 여부와 정배에 10만원 걸었을 때 손익

보내는 방법
- 수집기가 `/notify` 를 부르면 Worker 가 알림을 만들어 큐(`alert_queue`)에 쌓습니다
- Claude 루틴이 08시와 21~23시에 `/alerts/pending` 을 읽어 PlayMCP **카카오톡 나에게 보내기**로 보내고 `/alerts/ack` 로 지웁니다
- 큐 열쇠는 D1 `kv` 표의 `alert_key` 입니다. 12시간 넘게 못 보낸 알림은 버립니다

카카오 앱을 직접 만들어 연결하면(`KAKAO_REST_KEY` 비밀값과 `/kakao/login?key=INGEST_TOKEN값`) 루틴 없이 30분마다 바로 보냅니다.

## 저장 방식
- `proto_matches`: 경기의 게임 유형(승무패 핸디캡 언더오버 등) 하나당 한 행. 상태와 결과와 점수가 갱신됩니다
- `proto_odds`: 배당이 직전 값과 달라질 때만 한 줄씩 쌓입니다. 화면의 ▲▼ 는 첫 수집 대비 변동입니다

## 응답 구조 메모
`gameInfoInq.do` 는 `{"gmId":"G101","gmTs":260113}` 을 POST 하면 `compSchedules.keys` 와 `compSchedules.datas` 를 줍니다.
- `winAllot drawAllot loseAllot`: 배당 (0 이면 아직 미발표)
- `winHandi`: 핸디캡 또는 언더오버 기준값
- `gameResult`: 0 = 왼쪽(승 또는 언더) 1 = 무 2 = 오른쪽(패 또는 오버) 4 = 적특
- `protoStatus`: 4 = 결과 확정
