# 국내 프로토 배당 웹 (kl.usb.kr)

베트맨 프로토 승부식의 국내 경기 배당(축구 야구 농구 배구)을 모아 보여주는 페이지입니다.
위쪽 종목 버튼(`/?sport=야구` 등)으로 종목을 고릅니다.
...
- 국내 경기가 없는 회차에는 보낼 것이 없어서 대상 0행으로 끝납니다
...
## 분석용 내보내기
수집 대상은 베트맨이 국내로 표시한 모든 리그(K리그 KBO KBL WKBL V리그 코리아컵 등)와 toto 모델의 해외 리그 9개(EPL EFL챔 라리가 세리에A 분데스리 프리그1 에레디비 J1리그 MLS)입니다. 화면에는 국내 리그만 보입니다.
바뀐 것이 없는 경기는 다시 쓰지 않아서 종목이 늘어도 D1 무료 쓰기 한도 안에 들어옵니다.
```
...
## 예상 vs 결과
- 화면 아래 **배당 예상 vs 실제 결과** 에서 국내 리그 기본 게임(축구 승무패 야구 농구 배구 승패) 마감 배당이 실제로 얼마나 맞았는지 봅니다
- 예상은 마진을 뺀 배당 확률이고 실제는 적중 비율입니다. 차이가 + 면 배당이 예상한 것보다 자주 맞았다는 뜻입니다
- 수익률은 매 경기 같은 금액을 걸었을 때 기준입니다. 손익은 경기마다 10만원을 걸었을 때 총 금액입니다 (만원)
- 정배와 무승부와 역배 그리고 정배 배당 구간별과 시즌별로 나눠 보여 줍니다
- 끝난 경기 카드에는 `정배 적중` 또는 `정배 X 실패` 배지와 정배에 10만원 걸었을 때 손익이 붙습니다 (예: `10만원 → +134,000원`)
- 종목 버튼을 누르면 그 종목만 계산하고 리그별 정배 표도 나옵니다. 1시간마다 새로 계산합니다
- JSON 은 `/api/accuracy` (`?sport=야구`) 로 받습니다

## K리그 카카오톡 알림
수집기가 30분마다 돌 때 K리그 경기를 보고 카카오톡 **나와의 채팅**으로 보냅니다. 메시지는 모두 `[K리그]` 로 시작합니다.
- **새 배당**: 경기 배당이 처음 올라왔을 때
- **배당 변동**: 마지막으로 알린 값보다 승 무 패 중 하나가 0.1 이상 바뀌었을 때
